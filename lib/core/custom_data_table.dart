...
class CustomDataRow {
  ...
  ///
  /// If a [CustomDataCell] in the row has its [CustomDataCell.onTap], [CustomDataCell.onDoubleTap],
  /// [CustomDataCell.onLongPress], [CustomDataCell.onTapCancel] or [CustomDataCell.onTapDown] callback defined,
  /// that callback behavior overrides the gesture behavior of the row for
  /// that particular cell.
  ...
  bool get _debugInteractive =>
      onSelectChanged != null || cells.any((CustomDataCell cell) => cell._debugInteractive);
}
...
///
/// {@tool dartpad}
/// This sample shows how to display a [CustomDataTable] with three columns: name, age, and
/// role. The columns are defined by three [CustomDataColumn] objects. The table
/// contains three rows of data for three example users, the data for which
/// is defined by three [CustomDataRow] objects.
///
...
///
/// {@tool dartpad}
/// This sample shows how to display a [CustomDataTable] with alternate colors per
/// row, and a custom color for when the row is selected.
///
/// ** See code in examples/api/lib/material/data_table/data_table.1.dart **
/// {@end-tool}
///
/// [CustomDataTable] can be sorted on the basis of any column in [columns] in
...
class CustomDataTable extends StatelessWidget {
  ...
  /// selectable. However, if [CustomDataRow.onSelectChanged] is not set for any row,
  ...
  Widget _buildHeadingCell({
    ...
    final double effectiveHeadingRowHeight =
        headingRowHeight ?? themeData.dataTableTheme.headingRowHeight ?? _headingRowHeight;
    label = Container(
      padding: padding,
  ...
  Widget _buildDataCell({
    ...
    final TextStyle effectiveDataTextStyle =
        dataTextStyle ?? themeData.dataTableTheme.dataTextStyle ?? themeData.textTheme.bodyText2!;
    final double effectiveDataRowHeight =
        dataRowHeight ?? themeData.dataTableTheme.dataRowHeight ?? kMinInteractiveDimension;
    label = Container(
      padding: padding,
  ...
  Widget build(BuildContext context) {
    ...
    if (displayCheckboxColumn) {
      tableColumns[0] = FixedColumnWidth(effectiveCheckboxHorizontalMarginStart +
          Checkbox.width +
...
/// See also:
///
///  * [CustomDataTable], which makes use of [TableRowInkWell] when
