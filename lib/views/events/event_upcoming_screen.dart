...
class EventUpcomingScreen extends StatefulWidget {
  ...
  const EventUpcomingScreen(this.events, {Key? key}) : super(key: key);
  @override
...
class _EventUpcomingScreenState extends State<EventUpcomingScreen> {
  ...
  void _loadMore() async {
    if (_hasNextPage == true &&
      ...
      try {
        if (eventsProvider.upcomingEvents.length > 0) {
          print("past list in News home:");
          print(eventsProvider.upcomingEvents);
          setState(() {
            tempEvents.addAll(eventsProvider.upcomingEvents);
          });
        } else {
          print("No data");
