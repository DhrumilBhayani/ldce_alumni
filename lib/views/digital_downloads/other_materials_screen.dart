...
class OtherMaterialsScreen extends StatefulWidget {
  ...
  const OtherMaterialsScreen(this.events, {Key? key}) : super(key: key);
  @override
  _OtherMaterialsScreenState createState() => _OtherMaterialsScreenState();
}

class _OtherMaterialsScreenState extends State<OtherMaterialsScreen> {
  ...
  List<Widget> _buildEventsList() {
    ...
    if (widget.events.isEmpty) {
      print("List Empty");
      list.add(Container(padding: EdgeInsets.only(top: 10), child: Text("No Other Material.")));
    } else {
  ...
  void _loadMore() async {
    if (_hasNextPage == true &&
      ...
      try {
        if (eventsProvider.otherMaterials.length > 0) {
          print("past list in News home:");
          print(eventsProvider.otherMaterials);
          setState(() {
            tempEvents.addAll(eventsProvider.otherMaterials);
          });
        } else {
          print("No data");
