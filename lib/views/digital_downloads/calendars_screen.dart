...
class CalendarsScreen extends StatefulWidget {
  ...
  const CalendarsScreen(this.events, {Key? key}) : super(key: key);
  @override
...
class _CalendarsScreenState extends State<CalendarsScreen> {
  ...
  List<Widget> _buildEventsList() {
    ...
    if (widget.events.isEmpty) {
      print("List Empty");
      list.add(Container(padding: EdgeInsets.only(top: 10), child: Text("There no Calendar Available.")));
    } else {
  ...
  void _loadMore() async {
    if (_hasNextPage == true &&
      ...
      try {
        if (eventsProvider.calendars.length > 0) {
          print("past list in News home:");
          print(eventsProvider.calendars);
          setState(() {
            tempEvents.addAll(eventsProvider.calendars);
          });
        } else {
          print("No data");
