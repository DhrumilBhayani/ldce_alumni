...
class DesktopWallpapersScreen extends StatefulWidget {
  final List<DigitalDownloads> desktopWallpaper;
  const DesktopWallpapersScreen(this.desktopWallpaper, {Key? key}) : super(key: key);
  @override
  _DesktopWallpapersScreenState createState() => _DesktopWallpapersScreenState();
}

class _DesktopWallpapersScreenState extends State<DesktopWallpapersScreen> {
  late CustomTheme customTheme;
  late ThemeData theme;
  late ScrollController _controller;
  late List<DigitalDownloads> tempDesktopWallpaper;
  late DigitalDownloadsController digitalDownloadsProvider;
  ...
  void initState() {
    ...
    digitalDownloadsProvider = DigitalDownloadsController();
  ...
  Widget build(BuildContext context) {
    ...
    //   statusBarIconBrightness: Brightness.light
    // ));
    tempDesktopWallpaper = widget.desktopWallpaper;
    return ListView(
  ...
  List<Widget> _buildEventsList() {
    ...
    print("desktop wallpaper List");
    print(widget.desktopWallpaper);
    for (var item in widget.desktopWallpaper) {
      print(item.title);
    }
    List<Widget> list = [];
    // newsController = FxControllerStore.putOrFind<NewsController>(NewsController());
    if (widget.desktopWallpaper.isEmpty) {
      print("List Empty");
      list.add(Container(
          padding: EdgeInsets.only(top: 10), child: Text("There is no Desktop Wallpaper Available.")));
    } else {
      for (DigitalDownloads singleNews in widget.desktopWallpaper) {
  ...
  void _loadMore() async {
    if (_hasNextPage == true &&
        ...
        digitalDownloadsProvider.hasMoreUpcomingData == true &&
        _controller.position.extentAfter < 300 &&
        widget.desktopWallpaper.isNotEmpty &&
      ...
      print(' _loadmoreHas More Data:' + digitalDownloadsProvider.hasMoreUpcomingData.toString());

      await digitalDownloadsProvider.loadMore(_page, 'upcoming');
      try {
        if (digitalDownloadsProvider.desktopWallpapers.length > 0) {
          print("past list in News home:");
          print(digitalDownloadsProvider.desktopWallpapers);
          setState(() {
            tempDesktopWallpaper.addAll(digitalDownloadsProvider.desktopWallpapers);
          });
        } else {
          print("No data");
          // This means there is no more data
          // and therefore, we will not send another GET request
          digitalDownloadsProvider.hasMoreUpcomingData = false;
          setState(() {
            _hasNextPage = false;
          });
        }
